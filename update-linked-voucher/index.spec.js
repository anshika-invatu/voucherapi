...
describe('Update voucher', () => {
    ...
    it('should return status code 400 when request body is null', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${merchantID}/voucher/${sampleVoucher._id}`, {
                json: true,
                headers: {
    ...
    it('should throw error on incorrect _id field', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${merchantID}/voucher/123`, {
                json: true,
                body: {},
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY
    ...
    it('should throw 404 error if the documentId is invalid', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${merchantID}/voucher/${uuid.v4()}`, {
    ...
    it('should update document when all validation passes', async () => {
        const result = await request.patch(`${helpers.API_URL}/api/v1/merchants/${sampleVoucher.issuer.merchantID}/voucher/${sampleVoucher._id}`, {
            body: { isRedeemed: true },
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY
    ...
    after(async () => {
        ...
        if (result.deletedCount === 0) {
            console.log('voucherLog not deleted');
        }
