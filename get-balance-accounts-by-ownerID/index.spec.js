...
describe('Get Balance Accounts By OwnerID', () => {
    before(async () => {
        sampleBalanceAccount.partitionKey = sampleBalanceAccount._id;
        sampleBalanceAccount.balanceAccountType = 'balance';
    ...
    it('should return error when merchantID is invalid', async () => {
        try {
            await request.get(helpers.API_URL + `/api/v1/merchants/${123}/balance-accounts-docs`, {
                json: true,
                headers: {
    ...
    it('should return empty array if balanceAccount not exist of specified merchantID', async () => {
        ...
        const result = await request.get(helpers.API_URL + `/api/v1/merchants/${uuid.v4()}/balance-accounts-docs`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY
            }
    ...
    it('should return document when all validation passes', async () => {
        const balanceAccount = await request.get(helpers.API_URL + `/api/v1/merchants/${sampleMerchantID}/balance-accounts-docs`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY
            }
