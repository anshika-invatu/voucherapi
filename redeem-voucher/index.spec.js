...
const sampleVoucher = { ...require('../spec/sample-docs/Vouchers'), _id: uuid.v4(), passToken: utils.hashToken(passToken), isMultiFunctionVoucher: true, balanceAccountID: uuid.v4(), voucherToken: uuid.v4() };
...
describe('redeem-voucher', () => {
    ...
    describe('Validations', () => {
        it('should return status code 400 when request body is null', async () => {
            try {
                await request.put(`${helpers.API_URL}/api/v1/vouchers/redeem`, {
                    json: true,
                    headers: {
                        'x-functions-key': process.env.X_FUNCTIONS_KEY
                    }
                ...
                const response = {
                    code: 400,
                    description: 'You\'ve requested to redeem a voucher but the request body seems to be empty. Kindly specify the voucher properties to be redeemded using request body in application/json format',
        ...
        it('should return status code 400 when voucherToken is missing from queryParams', async () => {
            try {
                await request.put(`${helpers.API_URL}/api/v1/vouchers/redeem?passToken=123`, {
                    json: true,
                    headers: {
                        'x-functions-key': process.env.X_FUNCTIONS_KEY
                    },
        ...
        it('should validate voucherToken to match UUID v4 format', async () => {
            try {
                let url = `${helpers.API_URL}/api/v1/vouchers/redeem?passToken=123`;
        ...
        it('should validate if the voucher has already been redeemed', async () => {
            try {
                ...
                let url = `${helpers.API_URL}/api/v1/vouchers/redeem?passToken=${testVoucher.passToken}`;
    ...
    describe('Monetary voucher', () => {
        ...
        it('should update fixedAmount if merchantID not provided in request body', async () => {
    ...
    describe('Exchange voucher', () => {
        ...
        it('should allow case insensitive passToken match', async () => {
            ...
            let url = `${helpers.API_URL}/api/v1/vouchers/redeem?passToken=${passToken}`;
            url += `&voucherToken=${testVoucher.voucherToken}`;
            await request.put(url, {
                json: true,
                headers: {
