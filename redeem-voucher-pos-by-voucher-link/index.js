...
module.exports = async (context, req) => {
    ...
    let id, key, settlementTransaction, balanceAccountID, collection, merchant, voucher, voucherValue, merchantPartnerNetworks,
        partnerNetwork, voucherBalanceAmount, updatedVoucher, redemptionCost, balanceAccountDoc, updateResult, redemptionDoc, result;
    ...
    try {
            ...
            if (settlementList.settleValueOnRedemption === 'redeemedvalue') {
                ...
                if (!req.body.fixedAmount) {
                    if (voucherValue.valueType === 'exchange' || voucherValue.valueType === 'discount') {
                        ...
                        return utils.setContextResError(context,
        ...
        balanceAccountDoc = await collection.findOne({
        ...
        if (updatedVoucher) {
            ...
            const redemption = await redemptionUtils
            ...
            if (result === false) {
                redemptionDoc = collection.insertOne(redemption);
            } else if (result === true) {
                redemptionDoc = true;
            }
            if (redemptionDoc) {
