...
describe('redeem-voucher-merchant', () => {
    before(async () => {
    ...
    describe('Validations', () => {
        it('should return status code 400 when request body is null', async () => {
            try {
                await request.put(`${helpers.API_URL}/api/v1/vouchers/redeem-voucher-merchant`, {
                    json: true,
                    headers: {
                        'x-functions-key': process.env.X_FUNCTIONS_KEY
                    }
                ...
                const response = {
                    code: 400,
                    description: 'You\'ve requested to redeem a merchant_voucher but the request body seems to be empty. Kindly specify the merchant_voucher properties to be redeemded using request body in application/json format',
        ...
        it('should return status code 400 when voucherToken is missing from queryParams', async () => {
            try {
                await request.put(`${helpers.API_URL}/api/v1/vouchers/redeem-voucher-merchant?passToken=123`, {
                    json: true,
                    headers: {
                        'x-functions-key': process.env.X_FUNCTIONS_KEY
                    },
        ...
        it('should validate voucherToken to match UUID v4 format', async () => {
            try {
                let url = `${helpers.API_URL}/api/v1/vouchers/redeem-voucher-merchant?passToken=123`;
        ...
        it('should validate if the voucher has already been redeemed', async () => {
            try {
                ...
                let url = `${helpers.API_URL}/api/v1/vouchers/redeem-voucher-merchant?passToken=${testVoucher.passToken}`;
    ...
    describe('Exchange voucher', () => {
        ...
        it('should allow case insensitive passToken match', async () => {
            ...
            let url = `${helpers.API_URL}/api/v1/vouchers/redeem-voucher-merchant?passToken=${passToken}`;
            url += `&voucherToken=${testVoucher.voucherToken}`;
            await request.put(url, {
                json: true,
                headers: {
